...
queue-start: ## Start the Queue worker via systemctl
	@sudo systemctl start onyx-worker 2>/dev/null && echo "✅ Queue worker started" || echo "❌ Failed to start Queue worker (service may not exist)"

queue-stop: ## Stop the Queue worker via systemctl
	@sudo systemctl stop onyx-worker 2>/dev/null && echo "✅ Queue worker stopped" || echo "⚠️  Queue worker not stopped (service may not exist)"

queue-restart: ## Restart the Queue worker via systemctl
	@sudo systemctl restart onyx-worker 2>/dev/null && echo "✅ Queue worker restarted" || echo "❌ Failed to restart Queue worker (service may not exist)"

queue-status: ## Show the status of the Queue worker via systemctl
	@sudo systemctl status onyx-worker 2>/dev/null || echo "⚠️  Queue worker service not found"

queue-logs: ## Show the last 50 lines of Queue worker logs
	@sudo journalctl -u onyx-worker -n 50 --no-pager || echo "⚠️  Could not retrieve logs"
...
stop-and-wait-queue: ## Stop Queue worker and wait until fully stopped (max 30s)
	...
	sudo systemctl stop onyx-worker 2>/dev/null || true; \
	echo "Waiting for onyx-worker to fully stop..."; \
	i=0; \
	while [ "$$(systemctl is-active onyx-worker 2>/dev/null)" = "active" ] && [ $$i -lt 30 ]; do \
		sleep 1; i=$$((i+1)); \
	done; \
	if [ $$i -ge 30 ]; then \
		echo "⚠️  onyx-worker still active after 30s, continuing anyway"; \
	else \
		echo "✅ onyx-worker stopped"; \
	fi
...
deploy-prod: ## Deploy to production (requires git tag on HEAD, use FORCE=1 to bypass up-to-date check)
	...
	sudo systemctl restart onyx-worker 2>/dev/null || echo "⚠️  Queue worker not restarted (may not exist)" | tee -a $$LOG_FILE; \
	if [ $$DEPLOY_EXIT -eq 0 ]; then \
		APP_VERSION=$$(cat VERSION 2>/dev/null || echo "unknown"); \
		echo "✅ Deployed $$APP_VERSION successfully. Check worker status with: sudo systemctl status onyx-worker" | tee -a $$LOG_FILE; \
